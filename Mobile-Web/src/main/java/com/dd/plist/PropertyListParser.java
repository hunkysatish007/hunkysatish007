...
public class PropertyListParser {
	...
	/**
	 * Parses a property list from a file. It can either be in XML or binary
	 * format.
	 * 
	 * @param f
	...
	/**
	 * Parses a property list from a byte array. It can either be in XML or
	 * binary format.
	 * 
	 * @param bytes
	...
	public static NSObject parse(byte[] bytes) throws Exception {
		...
		if (magic_string.startsWith("bplist00")) {
			...
			throw new
			UnsupportedOperationException("The given data is neither a binary nor a XML property list. ASCII property lists are not supported.");
		}
	}

	/**
	 * Parses a property list from an InputStream. It can either be in XML or
	 * binary format.
	 * 
	 * @param is
	 *            The InputStream delivering the property list data
	...
	public static NSObject parse(InputStream is) throws Exception {
		if(is.markSupported()) {
			...
			if (is.available() > Runtime.getRuntime().freeMemory()) {
				throw new Exception("To little heap space available! Wanted to read " +
						is.available() + " bytes, but only " + Runtime.getRuntime().freeMemory()
						+ " are available.");
			}
