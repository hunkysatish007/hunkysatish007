//This Java File generate the LocalDb list view page
...
public class DbTableViewListBuilder {

	private static String getViewType(String viewType, BaseView baseview) {
		if (viewType.equalsIgnoreCase("DbTableViewList")) {
			return "DBTableViewList";
	...
	public static StringBuilder build(BaseView baseview, GenApp genApp) {
		...
		if (((String) child.getCellStyle()).equalsIgnoreCase("custom")) {
			...
			if (child.getGroup().get(0).getRecordCellDef().getEvent() != null) {
				...
				|| ((String) child.getCellStyle()).equalsIgnoreCase("right-aligned")
				|| ((String) child.getCellStyle()).equalsIgnoreCase("default")
				|| ((String) child.getCellStyle()).equalsIgnoreCase("contact-form")) {
			...
			for (Item group : child.getGroup()) {
				...
				if(tableRow.isAlternateRowStyle()){
					...
					contents.append("repeatImage:");
					contents.append(tableRow.isRowBGImageRepeat())
							.append(",\n\t");
				}
			}
		...
		} else if (((String) child.getCellStyle())
				.equalsIgnoreCase("simple-grid") || ((String) child.getCellStyle())
				.equalsIgnoreCase("tabular-grid")) {
			contents.append("{id:\"")
					.append(child.getUiid())
					.append("\",\n\t")
					.append("viewtype:\"")
			...
			for (Item group : child.getGroup()) {
				...
				System.out.println("TEST : "
						+ tableRow.getGridArray().get(0).getFieldName());
				contents.append("maintext:\"");
				contents.append(tableRow.getMainText())
						.append("\"")
						.append(",subtext:\"")
						.append(tableRow.getDetailText())
						.append("\",image:\"")
						.append(img)
						.append("\",")
						.append("backgroundColor:{")
						.append(ColorBuilder.build(tableRow
								.getBackgroundColor())).append("},\n\t");
				float updatedX = 0;
				...
				for (Grid grid : tableRow.getGridArray()) {
					...
					contents.append("{");
					contents.append("id:\"")
							.append(grid.getFieldName() + "_" + grid.getId())
							.append("\",");
					contents.append("name:\"").append(grid.getFieldName())
							.append("\",");
					...
					contents.append("value:\""
							+ grid.getFieldName()
							+ "\",template:\"\",viewtype:\"Label\",frame:{x:\""
							+ updatedX
							+ "\",y:\"0.0\",width:\""
							+ grid.getWidth()
							+ "\",height:\"44.0\"},border:{borderweight:\"1\",borderstyle:\"\",bordercolor:{red:0,blue:0,green:0,alpha:1.0}},background:{red:255,blue:255,green:255,alpha:0.0},padding:{top:5.0,bottom:5.0,left:5.0,right:5.0},font:{size:\"14\",align:\"left\",color:{red:0,blue:0,green:0,alpha:1.0}}},\n\t");
				...
				for (Grid grid : tableRow.getGridArray()) {
					...
					contents.append("row:\"").append(grid.getRow())
					.append("\",");
					contents.append("value:\"\",template:\"["
							+ grid.getFieldName()
							+ "]\",viewtype:\"Label\",frame:{x:\""
							+ updatedX
							+ "\",y:\"0.0\",width:\""
							+ grid.getWidth()
							+ "\",height:\"44.0\"},border:{borderweight:\"1\",borderstyle:\"\",bordercolor:{red:0,blue:0,green:0,alpha:1.0}},background:{red:255,blue:255,green:255,alpha:0.0},padding:{top:5.0,bottom:5.0,left:5.0,right:5.0},font:{size:\"14\",align:\"left\",color:{red:0,blue:0,green:0,alpha:1.0}}},\n\t");
